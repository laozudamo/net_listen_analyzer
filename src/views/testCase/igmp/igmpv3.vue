...
import { listv3 as list, create, del, update, copy, details } from '@/api/testcase/igmp.js'
...
const theForm = reactive({
  case_name: '',
  protocol_server_type: "igmp_v3", // one_stream more_stream, multi_stream
  total_time: 10,
  case_type: 5,
  config: {
    pkt_config: {
...
const caseName = () => {
  const date = new Date()
  theForm.case_name = 'IGMP_V3' + date.getFullYear() + (date.getMonth() + 1) + date.getDate() + date.getHours() + date.getMinutes() + date.getSeconds()
...
<template>
  ...
  <TheModel :title="isEdit ? '编辑IGMP/V3实例' : '新增IGMP/V3实例'" ref="model" @after-leave="afterLeave" width="900px"
      ...
      <n-tab-pane name="setting" tab="配置" display-directive="show">
        <n-form :model="theForm" label-align="left" label-placement="left" :style="{
          ...
          <n-form-item label="最大响应时间">
            <n-input v-model:value="theForm.config.pkt_config.mrcode" />
          </n-form-item>
