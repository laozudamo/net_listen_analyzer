...
import { listv2 as list, create, del, update, copy, details } from '@/api/testcase/igmp.js'
...
const theForm = reactive({
  case_name: '',
  protocol_server_type: "igmp_v2", // one_stream more_stream, multi_stream
  total_time: 10,
  case_type: 5,
  config: {
    pkt_config: {
...
const caseName = () => {
  const date = new Date()
  theForm.case_name = 'IGMP_V2' + date.getFullYear() + (date.getMonth() + 1) + date.getDate() + date.getHours() + date.getMinutes() + date.getSeconds()
...
<template>
  ...
  <TheModel :title="isEdit ? '编辑IGMP/V2实例' : '新增IGMP/V2实例'" ref="model" @after-leave="afterLeave" width="900px"
