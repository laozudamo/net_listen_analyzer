...
import { list, create, del, update, copy, details } from '@/api/testcase/tcp.js'
...
const caseName = () => {
  const date = new Date()
  theForm.case_name = 'tcp' + date.getFullYear() + (date.getMonth() + 1) + date.getDate() + date.getHours() + date.getMinutes() + date.getSeconds()
...
<template>
  ...
  <TheModel :title="isEdit ? '编辑RDMA实例' : '新增RDMA实例'" ref="model" @after-leave="afterLeave" width="900px"
    ...
    <n-tabs type="line" default-value="netCfg" size="medium" animated style="margin: 0px"
      ...
      <n-tab-pane name="netCfg" tab="网络拓扑" display-directive="show">
        <NetCfg :caseType="2" ref="netCfg" :topoData="topoData"></NetCfg>
      </n-tab-pane>
      ...
      <n-tab-pane name="setting" tab="配置" display-directive="show">
        <n-form :model="theForm" label-align="left" label-placement="left" :style="{
          ...
          <n-form-item label="dport">
            <n-input v-model:value="theForm.config.pkt_config.sport" />
          </n-form-item>
          <n-form-item label="sport">
            <n-input v-model:value="theForm.config.pkt_config.dport" />
          </n-form-item>
