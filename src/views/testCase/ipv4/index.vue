...
import { list, create, del, update, copy, details } from '@/api/testcase/ipv4.js'
...
const caseName = () => {
  const date = new Date()
  theForm.case_name = 'IPV4' + date.getFullYear() + (date.getMonth() + 1) + date.getDate() + date.getHours() + date.getMinutes() + date.getSeconds()
...
<template>
  ...
  <TheModel :title="isEdit ? '编辑IPV4实例' : '新增IPV4实例'" ref="model" @after-leave="afterLeave" width="900px"
    ...
    <n-tabs type="line" default-value="netCfg" size="medium" animated style="margin: 0px"
      ...
      <n-tab-pane name="netCfg" tab="网络拓扑" display-directive="show">
        <NetCfg :caseType="4" ref="netCfg" :topoData="topoData"></NetCfg>
      </n-tab-pane>
