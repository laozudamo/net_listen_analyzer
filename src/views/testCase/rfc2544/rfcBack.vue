...
const { list,
  ...
  del,
  copy,
  details } = getRfc("BackToBackFrames")
...
const theForm = reactive({
  case_name: '',
  protocol_server_type: "BackToBackFrames", // one_stream more_stream, multi_stream
  total_time: 10,
  case_type: 10,
  config: {
    pkt_config: {
...
function setFormVal (data) {
  ...
  tagsVal.value = data.config.stream_config.size_list.map(item => +item)
}
...
<template>
  <Title title="RFC背靠背" showTip="true" des="背靠背测试通常用于评估网络设备的最大吞吐量。它的目标是确定在最大传输速率下，网络设备是否能够维持稳定的性能，而不会丢失数据包或降低性能。
在背靠背测试中，测试设备以最大可能的速率发送数据包或帧到被测设备，以测试被测设备的极限性能。这通常会导致丢包，因为被测设备可能无法处理如此高的负载。
背靠背测试不会测量抖动或延迟，它主要集中在吞吐量和丢包率上。" />
  <SearchBox :searchForm="searchForm" @filterData="filterData(searchForm)"
    @refreshData="() => { refresh(); resetSearch() }">
  </SearchBox>
  ...
  <TheModel :title="isEdit ? '编辑RFC背靠背实例' : '新增RFC背靠背实例'" ref="model" @after-leave="afterLeave" width="900px"
    ...
    <n-tabs type="line" default-value="netCfg" size="medium" animated style="margin: 0px"
      ...
      <n-tab-pane name="setting" tab="配置" display-directive="show">
        <n-form :model="theForm" label-align="left" label-placement="left" :style="{
          ...
          <!-- <div class="the-dymac">
            <div style="width: 120px;">单包速率(pps)</div>
            <n-dynamic-tags v-model:value="tagsVal" />
          </div>

          <div class="the-dymac">
            <div style="width: 120px;">单包速率(pps)</div>
            <n-input-number :min="0" v-model:value="theForm.config.stream_config[0].pps" />
          </div>

          <div class="the-dymac">
            <div style="width: 120px;">包间隔(isg)</div>
            <n-input-number :min="0" v-model:value="theForm.config.stream_config[0].isg" />
          </div> -->
