...
const theForm = reactive({
  case_name: '',
  protocol_server_type: "FrameLossRate", // one_stream more_stream, multi_stream
  total_time: 10,
  case_type: 10,
  config: {
    pkt_config: {
...
<template>
  <Title title="RFC帧丢失" showTip="true" des="帧丢失测试用于测量网络设备在传输数据时是否会丢失数据帧或数据包。帧丢失通常以百分比来表示。
在帧丢失测试中，测试设备生成一系列数据帧并将其发送到被测设备。测试工具会记录接收到的数据帧数量以及丢失的数据帧数量，然后计算帧丢失率（通常以百分比表示）。
帧丢失测试有助于评估设备是否能够在高负载条件下保持数据的完整性，避免数据帧的丢失，因为帧丢失可能会导致数据传输中断或损坏。
" />
  ...
  <TheModel :title="isEdit ? '编辑RFC帧丢失实例' : '新增RFC帧丢失实例'" ref="model" @after-leave="afterLeave" width="900px"
