...
import { list, create, del, update, copy, details } from '@/api/testcase/arp.js'
    ...
    render: (row) => {
      return (
        <div>
          ...
          <BtnIcon disabled={row.run_status} color="#2969FF" msg="复制" type="default" onClick={() => copyData(row)}>
            <Copy />
          </BtnIcon>
          {
            row.run_status ?
...
const theForm = reactive({
  ...
  config: {
    pkt_config: {
      ...
      arp_op: 1 // 1：ARP请求，2：ARP应答，3：RARP请求，4：RARP应答
    },
    stream_config: [
      {
        size: 60,
...
const caseName = () => {
  const date = new Date()
  theForm.case_name = 'ARP' + date.getFullYear() + (date.getMonth() + 1) + date.getDate() + date.getHours() + date.getMinutes() + date.getSeconds()
...
<template>
  ...
  <TheModel :title="isEdit ? '编辑ARP实例' : '新增ARP实例'" ref="model" @after-leave="afterLeave" width="900px"
    ...
    <n-tabs type="line" default-value="netCfg" size="medium" animated style="margin: 0px"
      ...
      <n-tab-pane name="netCfg" tab="网络拓扑" display-directive="show">
        <NetCfg :caseType="7" ref="netCfg" :topoData="topoData"></NetCfg>
      </n-tab-pane>
      ...
      <n-tab-pane name="setting" tab="配置" display-directive="show">
        <n-form :model="theForm" label-align="left" label-placement="left" :style="{
          ...
          <n-form-item label="ARP类型">
            <n-radio-group v-model:value="theForm.config.pkt_config.arp_op" name="arpOp">
              <n-space>
                <n-radio :value="ele.value" v-for="(ele) in arpOption" :key="ele.value">
                  {{ ele.label }}
                </n-radio>
              </n-space>
            </n-radio-group>
