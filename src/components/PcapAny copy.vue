...
const menuConfig = reactive({
  body: {
    options: [
      [
        { code: 'myLink', name: '追踪TCP', disabled: false },
        { code: 'myLink', name: '追踪UDP', disabled: false },
        { code: 'myLink', name: '追踪TCP', disabled: false },
        { code: 'myLink', name: '追踪TCP', disabled: false },
        { code: 'myLink', name: '追踪TCP', disabled: false },
      ]
...
async function loadData () {
  try {
    ...
    list.value = data
    let tmp = data.splice(0, 30)
    await $table.loadData(tmp)
    loading.value = false
  } catch (error) {
    loading.value = false
  }
...
<template>
  <splitpanes horizontal class="default-theme" :dbl-click-splitter="false">
    <pane style="background-color:#FFF" class="left-pane">
      <div style="height: 100%;background: #FFF;padding: 20px;" ref="midEl">
        ...
        <vxe-toolbar ref="toolBar" refresh>
          <template #tools>
            <vxe-input style="width: 500px;margin-right: 20px;" v-model="filterName" type="search" placeholder="试试全表搜索"
        ...
        <vxe-table :menu-config="menuConfig" @current-change="currentChange" :loading="loading" show-overflow keep-source
          ref="xTable" border height="430" :row-config="{ isHover: true, isCurrent: true, useKey: true }"
          :column-config="{ useKey: true }" :scroll-y="{ enabled: true, gt: 20, scrollToTopOnChange: true }">
    ...
    <pane min-size="0" max-size="40" size="40" class="left-pane">
