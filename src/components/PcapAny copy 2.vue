...
<template>
  <div style="padding: 30px;">
    <video width="200" height="240" controls v-for="(item,i) in 30" :key="i" >
...
<template>
    ...
    <template #buttons>
      <vxe-button>
        ...
        <template #dropdowns>
          ...
          <vxe-button type="text" @click="insertEvent($refs.xTable.getData(200))">插入到 200 行</vxe-button>
        </template>
      </vxe-button>
      <vxe-button>
        ...
        <template #dropdowns>
          ...
          <vxe-button type="text"
            @click="$refs.xTable.remove($refs.xTable.getData($refs.xTable.getData().length - 1))">删除最后一行</vxe-button>
      ...
      <vxe-button>
        ...
        <template #dropdowns>
          <vxe-button type="text" @click="$refs.xTable.scrollToRow($refs.xTable.getData(0))">滚动到第 10 行</vxe-button>
          <vxe-button type="text" @click="$refs.xTable.scrollToRow($refs.xTable.getData(400))">滚4动第 400 行</vxe-button>
  ...
  <vxe-table border show-overflow keep-source ref="xTable" height="300" :column-config="{ resizable: true }"
    :loading="demo1.loading" :edit-rules="demo1.validRules" :mouse-config="{ selected: true }"
